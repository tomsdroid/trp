<template>
  <div class="min-h-screen bg-slate-50 flex flex-col pb-[90px]">
    ...
    <div class="bg-teal-600 px-4 py-5 shadow-sm text-white sticky top-0 z-20 flex items-center gap-3">
      ...
      <div>
        <h1 class="text-lg font-bold leading-tight">Kelola Area</h1>
    ...
    <div class="p-4 flex-1">
      <div v-if="areaList.length === 0" class="text-center py-10">
        <div class="w-16 h-16 bg-slate-200 rounded-full flex items-center justify-center mx-auto mb-3 text-slate-400">
          <MapPinIcon :size="32" />
        </div>
        <p class="text-sm font-bold text-slate-600">{{ isLoading ? 'Memuat data...' : 'Belum ada area' }}</p>
        <p v-if="!isLoading" class="text-xs text-slate-400 mt-1">Silakan tambah area baru.</p>
      </div>
        ...
        <div v-for="area in areaList" :key="area.id" 
          ...
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full flex items-center justify-center"
                 :class="area.is_active == 1 ? 'bg-indigo-50 text-indigo-500' : 'bg-slate-100 text-slate-400'">
              <MapPinIcon :size="20" />
            </div>
            <div>
              <h3 class="font-bold text-slate-800" :class="{'line-through text-slate-400': area.is_active == 0}">
                {{ area.name }}
              </h3>
              <span class="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    :class="area.is_active == 1 ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-500'">
                {{ area.is_active == 1 ? 'Aktif' : 'Nonaktif' }}
              </span>
            </div>
          </div>
          <!-- Tombol Edit -->
          <button @click="bukaModalEdit(area)" class="p-2 text-blue-500 bg-blue-50 rounded-xl active:scale-95 transition-transform">
            <Edit2Icon :size="16" />
          </button>
        </div>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end justify-center z-50">
      <div class="bg-white w-full rounded-t-3xl p-6 shadow-2xl modal-slide-up">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-lg font-bold text-slate-800">{{ isEdit ? 'Edit Area' : 'Tambah Area' }}</h3>
        ...
        <form @submit.prevent="simpanData" class="space-y-4">
          <div>
            <label class="text-xs font-bold text-slate-500 mb-1 block">Nama Area</label>
            <input v-model="formData.name" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:border-teal-500" placeholder="Misal: Palembang Kota" required>
          </div>
          ...
          <div v-if="isEdit" class="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
            <div>
              ...
              <p class="text-[10px] text-slate-500">Area nonaktif tidak akan muncul di HP Terapis.</p>
            </div>
...
const loadData = async () => {
  ...
  try {
    ...
    if (result.status === 'success' && result.data.areas) {
      areaList.value = result.data.areas.sort((a, b) => a.name.localeCompare(b.name));
    }
  } catch (error) {
    console.error(error);
  } finally {
...
const bukaModalEdit = (area) => {
  isEdit.value = true;
  formData.value = { id: area.id, name: area.name, is_active: area.is_active == 1 };
  showModal.value = true;
};
...
const simpanData = async () => {
  try {
    ...
    const response = await fetch(`${API_URL}/sync_areas.php`, {
      method: 'POST',
